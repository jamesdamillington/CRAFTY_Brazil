 ...
 * 
 */
public class DoubleCroppingPropagation implements CapitalUpdateFunction {
	...
	public DoubleCroppingPropagation() {
	};

	public DoubleCroppingPropagation(Capital c, double change) {
		this.capital = c;
		this.change = change;
	}

	public DoubleCroppingPropagation(Capital c, double change, double top, double bottom) {
		this(c, change);
		this.top = top;
		this.bottom = bottom;
	}
	...
	public void apply(Cell c, Region region, String year) {
		...
		if(!(region.getCell(x, y-1)==null)){
			if(region.getCell(x, y-1).getOwnersFrLabel().equals("FR3")){
				tested=true;
		...
		if(!(region.getCell(x-1, y)==null)){
			if(region.getCell(x-1, y).getOwnersFrLabel().equals("FR3")){
				tested=true;
