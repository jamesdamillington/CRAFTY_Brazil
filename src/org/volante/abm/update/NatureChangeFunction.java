 ...
 * 
 */
public class NatureChangeFunction implements CapitalUpdateFunction {
	...
	public NatureChangeFunction() {
	};

	public NatureChangeFunction(Capital c, double change) {
		this.capital = c;
		this.change = change;
	}

	public NatureChangeFunction(Capital c, double change, double top, double bottom) {
		this(c, change);
		this.top = top;
		this.bottom = bottom;
	}
	...
	public void apply(Cell c, Region region, String year) {
		...
		if(c.getBaseCapitals().getDouble(capital)>top){
			...
			double value = c.getBaseCapitals().getDouble(capital)-change;//decrease capital value by specified change value
			if(value<0){//if the thus incremented capital is now below 0
				value = 0;//set it to 0
			}
