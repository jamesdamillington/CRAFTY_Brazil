 ...
 * It checks if a neighbouring cell (Queen's case) is FR4 or FR5. If all neighbouring cells are in one of these FRs
 * then the Capital value for the cell is set to 'change' 
 */
public class ProportionalChangeFunction2 implements CapitalUpdateFunction {
	...
	public ProportionalChangeFunction2() {
	};

	public ProportionalChangeFunction2(Capital c, double change) {
		this.capital = c;
		this.change = change;
	}

	public ProportionalChangeFunction2(Capital c, double change, double top, double bottom) {
		this(c, change);
		this.top = top;
		this.bottom = bottom;
	}
	...
	public void apply(Cell c, Region region, String year) {
		...
		//System.out.println("Executing Prop Change Function 2");
		
		/**
		if(!(region.getCell(x, y-1)==null)){
			if(!region.getCell(x, y-1).getOwnersFrLabel().equals(c.getOwnersFrLabel())){
				tested=true;
			}
		}
		if(!(region.getCell(x, y+1)==null)){
		...
		if(!(region.getCell(x-1, y)==null)){
			if(!region.getCell(x-1, y).getOwnersFrLabel().equals(c.getOwnersFrLabel())){
				tested=true;
			}
		}
		if(!(region.getCell(x+1, y)==null)){
		...
		*/
		...
		if(!(region.getCell(x, y-1)==null)){
			if(region.getCell(x, y-1).getOwnersFrLabel().equals("FR4") || region.getCell(x, y-1).getOwnersFrLabel().equals("FR5")){
				tested=true;				
			}
			else { tested=false; }
		}
		...
		if(!(region.getCell(x-1, y)==null)){
			if(region.getCell(x-1, y).getOwnersFrLabel().equals("FR4") || region.getCell(x-1, y).getOwnersFrLabel().equals("FR5")){
				tested=true;				
			}
			else { tested=false; }
		}
		...
		if(!(region.getCell(x+1, y+1)==null)){
			if(region.getCell(x+1, y+1).getOwnersFrLabel().equals("FR4") || region.getCell(x+1, y+1).getOwnersFrLabel().equals("FR5")){
				tested=true;				
			}
			else { tested=false; }
		}
		...
		//if(c.getBaseCapitals().getDouble(capital)<top){
			...
			//System.out.println("Nature core - set 0.0");
			
			DoubleMap<Capital>	baseCapitals		= (DoubleMap<Capital>) c.getBaseCapitals();
			baseCapitals.put(capital, change);
